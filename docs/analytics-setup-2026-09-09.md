...
- Sitemap https://bee-smart.ai/sitemap.xml enviado y procesado. Ahora publica seis páginas, incluida `/agenda/`; Search Console puede tardar en volver a descubrirlas. Esto no significa que todas estén indexadas.
...
Eventos propios: `registration_click`, `business_form_click`, `booking_click`, `whatsapp_click` y `email_click`. Solo incluyen ubicación del botón y, cuando corresponde, un identificador de sesión permitido. Son clics de interés, no formularios completados, llamadas reservadas, pagos ni ventas. Google Forms y el calendario incrustado no confirman sus envíos a esta etiqueta.

Meta recibe `PageView`; en IA para Todos también recibe `ViewContent`; los clics permitidos se envían como `Contact` con el tipo de contacto y los accesos a la agenda como `Schedule`. No se utiliza `Lead` hasta poder confirmar un formulario enviado. Conversions API queda pendiente de un formulario propio y una ruta de servidor adecuada.
...
- Catorce pruebas de artefacto y analítica aprobadas; `npm test` incluye ambos conjuntos.
